...
7. **Траектория (AI vs Game) и Производительность**:
   - Для AI симуляции выстрела используется симулятор с substepping и проверкой коллизии по радиусу, чтобы не было рассинхрона с реальной физикой (особенно на высокой скорости).
   - Для CPU оптимизации проверки террейна используется предрасчитанный набор оффсетов “круга” для радиуса вместо двойного цикла по квадрату.
8. **Стратегия бота (MCTS в WebWorker)**:
   - Планирование хода выполняется в WebWorker, чтобы не блокировать главный поток.
   - Сложность (easy/medium/hard) задаётся бюджетом поиска и глубиной rollouts (а не только “шумом прицеливания”).
   - Для ускорения используется action abstraction (MoveTo + Shot) и progressive widening для непрерывных параметров (угол/сила).
...
### 3.3.1 Custom Maps (Admin)
- Загрузка пользовательской карты выполняется в админке (PNG → base64 → сохранение в KV + метаданные в D1).
- В админке доступен массовый апскейл всех custom maps до высоты 1000px с сохранением пропорций и перезаписью по текущему id (update).
